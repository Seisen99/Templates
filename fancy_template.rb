...
after_bundle do
  ...
  <div class="fixed top-0 right-0 m-2 mt-3">
    <% if user_signed_in? %>
      <%= link_to "Disconnect", destroy_user_session_path,  data: {turbo_method: :delete}, class: "bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full text-sm" %>
    <% else %>
      <%= link_to "Sign In", new_user_session_path, class: "bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full text-sm" %>
    <% end %>
  </div>
  ...
  class PagesList extends React.Component {
    ...
    render() {
      return (
        <div>
          <h1 className="text-element text-6xl text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600 pb-7 pl-4">Hello! Welcome to your new rails project!</h1>
          <p className="text-gray-400  text-2xl mt-4 p-4">In this app you can use TailwindCSS and Anime.js with React 🥳!</p>
          <p className="text-gray-400  text-2xl p-4">You can also use StimulusJS and Turbo too 🤯.</p>
          <p className="text-gray-400  text-2xl p-4">Have fun!</p>
        </div>
      );
    }
  }
